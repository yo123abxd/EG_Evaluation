...
float run(graph_t& G, typename graph_t::weight_type* bc_values) {
  ...
  thrust::host_vector<float> total_elapsed;
  std::size_t n_jobs = n_vertices;
  const std::size_t step = 1000;
  int base = 0;

  while (n_jobs > 0) {
    std::size_t curr_job_num = std::min(step, n_jobs);

    auto f = [&](std::size_t job_idx) -> float {
      return bc::run(G, (vertex_t)job_idx + base, bc_values);
    };


    total_elapsed.push_back(0.0);
    operators::batch::execute(f, curr_job_num, &(total_elapsed.back()));

    n_jobs -= curr_job_num;
    base += curr_job_num;
  }

  return std::accumulate(total_elapsed.begin(), total_elapsed.end(), 0.0);
}
